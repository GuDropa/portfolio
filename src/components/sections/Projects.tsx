...
function ComplexityIndicator({ level }: { level?: 'simple' | 'moderate' | 'complex' }) {
  const dotCount = level === 'complex' ? 3 : level === 'moderate' ? 2 : 1;

  return (
    <div className={styles.complexityIndicator} title={level || 'simple'}>
      {[...Array(3)].map((_, i) => (
        <div
          key={i}
          className={`${styles.complexityDot} ${i < dotCount ? styles.filled : ''}`}
        />
      ))}
    </div>
  );
          ...
          {items.map((project, i) => (
            ...
            >
              ...
              <div className={styles.cardInner}>
                {/* Header: complexity indicator + title + links */}
